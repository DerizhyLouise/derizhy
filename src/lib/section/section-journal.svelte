<script lang="ts">
    import SearchBar from "$lib/custom-components/search-bar.svelte";
    import { journalMenu } from "$lib/data/journal";
    import type { JournalMenu } from "$lib/type/data-type";
    import { typeCount } from "$lib/utils";

    let { isJournalPage = false }: { isJournalPage?: boolean } = $props();

    let search: string = $state("");
    let selectedType: string[] = $state(["All"]);

    let journalData: JournalMenu[] = $derived.by(() => {
        let filtered = journalMenu;

        if (!selectedType.includes("All")) {
            filtered = filtered.filter((item) =>
                item.type.some((t) => selectedType.includes(t)),
            );
        }

        if (search.trim().length > 0) {
            const query = search.toLowerCase();

            filtered = filtered.filter((item) => {
                const inTitle = item.title.toLowerCase().includes(query);
                const inDescription = item.description
                    ?.toLowerCase()
                    .includes(query);
                const inKeyword = item.keyword.some((d) =>
                    d.toLowerCase().includes(query),
                );
                const inType = item.type.some((t) =>
                    t.toLowerCase().includes(query),
                );

                return inTitle || inKeyword || inDescription || inType;
            });
        }

        return isJournalPage ? filtered : filtered.slice(0, 6);
    });
</script>

<section
    id="journal"
    class="h-full min-h-screen w-full px-10 text-white sm:px-20 xl:px-40 2xl:px-80"
>
    <div class="py-40">
        <h2
            class="text-gray mb-10 text-center text-6xl font-semibold uppercase"
            data-aos="fade-up"
            data-aos-duration="2000"
        >
            Journal
        </h2>
        {#if isJournalPage}
            <SearchBar
                bind:search
                bind:selectedType
                types={typeCount(journalMenu)}
                placeholder="Search Journal..."
            />
            <div
                class="text-gray mt-16 mb-5 ml-2"
                data-aos="fade-up"
                data-aos-duration="2000"
            >
                Showing <span class="font-semibold">{journalData.length}</span>
                journal{journalData.length > 1 ? "s" : ""} in
                <span class="text-maroon font-semibold">
                    {selectedType.join(", ")}
                </span>
            </div>
        {/if}
        <div
            class="text-gray mt-6 grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-8 xl:gap-16"
            data-aos="fade-up"
            data-aos-duration="2000"
        >
            {#each journalData as item (item.title)}
                <a
                    href={item.link}
                    class="bg-gray shadow-gray flex h-auto flex-col rounded-sm text-white shadow-lg"
                >
                    <div
                        class="bg-lightgray text-yellow flex h-60 items-center justify-center rounded-t-sm text-8xl"
                    >
                        {#if item.icon.type === "svg"}
                            <img
                                src={item.icon.src}
                                alt="Icon"
                                class="mx-auto h-32 w-32"
                            />
                        {:else}
                            <span class={item.icon.src}></span>
                        {/if}
                    </div>
                    <div class="flex grow flex-col p-6">
                        <div class="mb-2">
                            <div class="text-yellow text-xs">
                                <span class="fa-regular fa-calendar mr-1"
                                ></span>
                                {item.time}
                            </div>
                        </div>
                        <h3 class="text-2xl font-semibold">
                            {item.title}
                        </h3>
                        <p class="mt-2 text-sm text-gray-200 md:text-base">
                            {item.description}
                        </p>
                        <div class="text-yellow mt-2 flex grow items-end">
                            <div
                                class="transition-all duration-300 hover:translate-x-2.5"
                            >
                                Read More
                                <span class="fa-regular fa-chevron-right"
                                ></span>
                            </div>
                        </div>
                    </div>
                </a>
            {/each}
        </div>
        <div
            class="mt-20 text-center"
            data-aos="fade-up"
            data-aos-duration="2000"
        >
            {#if !isJournalPage}
                <a
                    href="/journal"
                    class="bg-maroon rounded-sm px-8 py-4 text-white transition-all duration-300 hover:brightness-125"
                >
                    View All Journals
                </a>
            {/if}
        </div>
    </div>
</section>
